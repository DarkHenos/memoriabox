...
export async function POST(request: NextRequest) {
  try {
    ...
    // Tentative d'envoi d'emails (interne + client)
    try {
      const emailResult = await sendContactEmail(data)
      
      // Message de succès adapté selon le résultat
      let successMessage = 'Merci ! Votre message a été envoyé.';
      
      if (emailResult.client.success) {
        successMessage += ' Vous allez recevoir un email de confirmation.';
      } else {
        successMessage += ' Nous vous répondrons rapidement.';
      }
